...
# Configure LLMs using environment variables for API keys
google_lm = dspy.Google(model="gemini-2.5-flash-preview-05-20", api_key=os.getenv("GOOGLE_API_KEY"))
openai_lm = dspy.OpenAI(model="gpt-4.1-2025-04-14", api_key=os.getenv("OPENAI_API_KEY"))
openrouter_lm = dspy.OpenRouter(model="google/gemini-2.0-flash-001", api_key=os.getenv("OPENROUTER_API_KEY"))
dspy.settings.configure(lm=openai_lm)  # Default to OpenAI for primary agent
...
class TesterAgent(dspy.Module):
    ...
    def forward(self, code):
        # Simulate running the code and checking usability
        try:
            # Basic syntax check
            ast.parse(code)
            # Additional usability checks (e.g., presence of user-friendly outputs)
...
class CodeDevelopmentSystem(dspy.Module):
    ...
    async def forward(self, user_request):
        ...
        while iteration < self.max_iterations:
            ...
            # Step 4: Simulate user review (in a real system, this would be actual user input)
    ...
    async def simulate_user_feedback(self, code, test_results, iteration):
        # Simulate user feedback (in practice, this would be real user input via a UI or API)
        print(f"\nPlease review the code and provide feedback or type 'approve' to accept:\n{code}\n")
        print(f"Test Results:\n{test_results}\n")
        # For demo purposes, simulate feedback based on iteration
        if iteration == 0:
