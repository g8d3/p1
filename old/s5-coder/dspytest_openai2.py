...
# Custom LM wrapper for OpenAI (if dspy.OpenAI is unavailable)
class OpenAICustomLM(dspy.LM):
...
# Configure DSPy with custom OpenAI LM
dspy.settings.configure(lm=OpenAICustomLM(model="gpt-4o", api_key=os.getenv("OPENAI_API_KEY")))
