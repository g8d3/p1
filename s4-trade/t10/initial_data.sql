...
('maintenance_mode', 'false', 'boolean', 'Global maintenance mode toggle'),
...
-- Link roles to permissions (example: Admin gets all, Trader gets strategy ones)
...
(1, 1), (1, 2), (1, 3), (1, 4),  -- All roles see dashboard
(2, 1), (2, 2), (2, 3),          -- Admin, Trader, Quant see strategy editor
(3, 1);                          -- Only Admin sees user management
...
-- Additional Initial Configuration Data
-- Insert sample users (wallet-based)
INSERT INTO Users (username, is_active) VALUES
('0xAb5801a7D398351b8bE11C439e05C5B3259aeC9B', TRUE),  -- Sample trader wallet
('0x742d35Cc6634C0532925a3b8D3D8f8e8b98a1E', TRUE); -- Sample admin wallet

-- Assign roles to users
INSERT INTO User_Roles (user_id, role_id) VALUES
(1, 2),  -- Trader role for first user
(2, 1);  -- Admin role for second user
...
('ETH Funding Rate Arb', 'Arbitrage between dYdX and GMX funding rates for ETH-PERP', 1, TRUE, 1),  -- TWAP
('BTC Cross-DEX Basis', 'Basis trade BTC between Binance and dYdX', 1, FALSE, 2);               -- Instant
...
-- Insert sample strategy rules (for first strategy: entry if spread > min)
...
-- Insert sample discovered opportunities (for demo)
INSERT INTO Discovered_Opportunities (strategy_id, market_a_id, market_b_id, calculated_apr, max_size_usd, expires_at) VALUES
(1, 1, 1, 0.085, 50000, NOW() + INTERVAL '1 hour'),  -- 8.5% APR opportunity
(2, 2, 1, 0.042, 25000, NOW() + INTERVAL '30 minutes'); -- 4.2% APR
...
(1, 'Aggressive ETH Arb Fund', 100000, 'USDT', '[{"rule": "max_pct_per_trade", "value": 0.25}, {"rule": "min_apr", "value": 60}]', TRUE, TRUE, 'Automated allocation to high-yield ETH funding arbs.');
...
-- This completes a functional initial dataset. The app engine can now query and operate on this data.
-- Next steps: Use tools like Directus to generate the CRUD web app on top of this schema.